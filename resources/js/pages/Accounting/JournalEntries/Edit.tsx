...
import { ChartOfAccount, JournalEntry, JournalItem } from '@/types';
...
export default function JournalEntriesEdit({
  journalEntry,
  accounts
}: JournalEntriesEditProps) {
  // Prepare initial form data from journal entry
  const initialItems: JournalItemForm[] = journalEntry.items.map(item => ({
    id: item.id,
  ...
  const { data, setData, put, processing, errors } = useForm({
    ...
    entry_date: journalEntry.entry_date.split('T')[0], // Format date for input
  ...
  const handleItemChange = (index: number, field: keyof JournalItemForm, value: string) => {
  ...
  return (
    <AppLayout title="Edit Journal Entry">
      <Head title="Edit Journal Entry - Tally Software" />
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Financial Year: {journalEntry.financialYear.name} ({new Date(journalEntry.financialYear.start_date).toLocaleDateString()} - {new Date(journalEntry.financialYear.end_date).toLocaleDateString()})
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit}>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`h-10 pl-10 px-4 py-2 shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 block w-full text-sm border-gray-300 rounded-md ${
                        errors.entry_date ? 'border-red-300 focus:ring-red-500 focus:border-red-500' : ''
                      }`}
                      required
                    />
                    {errors.entry_date && (
              ...
              <div>
                ...
                <textarea
                  ...
                  className={`px-4 py-2 shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 block w-full text-sm border-gray-300 rounded-md ${
                    errors.narration ? 'border-red-300 focus:ring-red-500 focus:border-red-500' : ''
                  }`}
                        ...
                        <tr key={index} className={item.type === 'debit' ? 'bg-green-50' : 'bg-blue-50'}>
                          <td className="px-6 py-4">
                            <select
                              ...
                              className={`w-full focus:ring-blue-500 focus:border-blue-500 text-sm border-gray-300 rounded-md ${
                                errors[`items.${index}.account_id`] ? 'border-red-300' : ''
                              }`}
                              required
                            >
                          ...
                          <td className="px-6 py-4">
                            <select
                              ...
                              className={`w-full focus:ring-blue-500 focus:border-blue-500 text-sm border-gray-300 rounded-md ${
                                errors[`items.${index}.type`] ? 'border-red-300' : ''
                              }`}
                              required
                            >
                          ...
                          <td className="px-6 py-4">
                            <input
                              ...
                              className={`w-full focus:ring-blue-500 focus:border-blue-500 text-sm border-gray-300 rounded-md ${
                                errors[`items.${index}.amount`] ? 'border-red-300' : ''
                              }`}
                              placeholder="0.00"
                              required
                            />
            ...
            <div className="mt-8 pt-5 border-t border-gray-200 flex justify-end gap-3">
              ...
              <button
                ...
                className={`px-6 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors ${
