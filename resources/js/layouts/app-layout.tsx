...
export default function MainLayout({ children, title = 'Dashboard' }: MainLayoutProps) {
    ...
    const menuItems: MenuItem[] = [
        ...
        // New Businesses item
        {
            ...
            submenu: [
                { label: 'All Businesses', href: '/businesses', icon: <Building size={18} /> },
                { label: 'Add Business', href: '/businesses/create', icon: <Copy size={18} /> },
                { label: 'Manage Users', href: '/admin/businesses/users', icon: <Users size={18} /> },
            ],
            roles: ['admin'], // Only admin can manage businesses
        },
                                ...
                                {item.submenu ? (
                                    <div>
                                        <button
                                            ...
                                            className={`flex items-center justify-between w-full px-4 py-2 text-sm font-medium rounded-md ${openSubmenus[item.label] ? 'bg-indigo-50 text-indigo-600' : 'text-gray-700 hover:bg-gray-100'
                                                }`}
                                        >
